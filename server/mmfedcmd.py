...
from client.mmfedcmd import FedCMDClient
...
class FedCMDServer(ServerBase):
    def __init__(self):
        super(FedCMDServer, self).__init__(get_args(), "FedCMD")
        fix_random_seed(self.args.seed)

        self.trainer = FedCMDClient(args=self.args,
        ...
        self.save_dir = self.this_file_dir / 'checkpoints' / self.algo / self.args.dataset / 'client-{}-cr-{}'. \
            ...
            self.args.local_epochs) / 'lr-{}'. \
                            format(self.args.local_lr) / 'CMD-{}'.format(str(self.args.cross_modal_distill)) / 'Mutual-Distill-False' / 'MLP-3layer'  # cwd: class-wise distillation

        self.log_dir = self.this_file_dir / 'logs' / self.algo / self.args.dataset / 'client-{}-cr-{}'. \
                           format(self.args.client_num, self.args.client_num_per_round) / 'alpha-{}-mmratio-{}' \
                           .format(self.args.alpha, self.args.multi_ratio) / 'local-epoch-{}'.format(
            self.args.local_epochs) / 'lr-{}'. \
                           format(self.args.local_lr) / 'CMD-{}'.format(str(self.args.cross_modal_distill)) /'Mutual-Distill-False' / 'MLP-3layer'
