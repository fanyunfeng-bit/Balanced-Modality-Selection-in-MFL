...
class FedCMDClient(ClientBase):
    def __init__(
            ...
            logger: Console,
            gpu: int, ):
        super(FedCMDClient, self).__init__(
            args,
            model,
            dataset,
            batch_size,
        ...
        self.coef2 = 2.0  # cross modal distillation
        self.coef3 = 2.0  #
    ...
    def _train(self):
        ...
        if self.modality_type == 'multimodal' and self.global_epoch >= self.args.warmup_epoch:
            ...
            average /= self.n_classes

            class_wise_dw = [0 for _ in range(self.n_classes)]
            class_wise_temp = [0 for _ in range(self.n_classes)]
        ...
        for le in range(self.local_epochs):
            ...
            for step, (spec, image, label) in enumerate(train_dataloader):
                ...
                if self.modality_type == 'multimodal':
                    ...
                    loss = criterion(out, label) + criterion(av_out[:label.shape[0]], label) + criterion(va_out[label.shape[0]:], label)
                ...
                if self.args.cross_modal_distill:
                        ...
                        if self.global_epoch >= self.args.warmup_epoch:
                            ...
                            if self.args.class_wise_t:
                                p_a, p_v = torch.zeros_like(av_out).to(self.device), torch.zeros_like(va_out).to(
                                    self.device)
                                for ll in range(label.shape[0]):
                                    p_a[ll] = F.softmax(av_out[ll] / self.args.temp, dim=0)
                        ...
                        if self.global_epoch >= self.args.warmup_epoch:
                            ...
                            if self.args.class_wise_t:
                                ...
                                for ll in range(label.shape[0]):
                                    ...
                                    # p_v[ll] = F.softmax(va_out[ll] / class_wise_temp[label[ll]], dim=0)
                                    p_v[label.shape[0] + ll] = F.softmax(
                                        va_out[label.shape[0] + ll] / self.args.temp, dim=0)
                    ...
                    loss += self.coef2 * (loss_rd + 0*loss_fd)
